function [x,converged,err_f_k,err_dx_k,iterations,X] = newton(eval_f,eval_Jf,p,u,x0,newton_opts);
   ...
   x = X(:,end);
   iterations = k-1; 
   if err_f_k(end)<=newton_opts.err_f & err_dx_k(end)<=newton_opts.err_dx
      converged = 1;
   else
      converged=0;
