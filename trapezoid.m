function [t,X] = trapezoid(eval_f,p,u,x0,tf,dt_max, Jf, trap_opts,newton_opts);
   ...
   while t(end) < tf
      ...
      if trap_opts.analytical_jacobian == true
        % Solve the system directly
        Jf_trap = speye(size(Jf, 1)) - p.dt/2*Jf;
        x = Jf_trap\(-f_trap(X(:,end),p,u_t));
      else
        [x,converged,err_f_k,err_dx_k,err_rel_k,k,k_gcr,~] = newton(f_trap, p, u_t, X(:,end) + dt_l*eval_f(X(:,end),p,u_t), newton_opts);
